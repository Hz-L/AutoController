...
BestS=X(1,:); %全局最优个体
for i=2:Size
    if chap8_18obj(X(i,:),y,N)<chap8_18obj(BestS,y,N)        
        BestS=X(i,:);
    end
end
Ji=chap8_18obj(BestS,y,N);
%进入主要循环，直到满足精度要求
for kg=1:1:G
     time(kg)=kg;
%变异
...
%选择        
        if(chap8_18obj(v(i,:),y,N)<chap8_18obj(X(i,:),y,N))
            X(i,:)=v(i,:);
        end
%判断和更新       
       if chap8_18obj(X(i,:),y,N)<Ji %判断当此时的指标是否为最优的情况
          Ji=chap8_18obj(X(i,:),y,N);
          BestS=X(i,:);
        end
    end
Best_J(kg)=chap8_18obj(BestS,y,N);
end
