...
for i=1:Size
    ...
    if i==1
        plocal(i,:)=chap8_8lbest(x(Size,:),x(i,:),x(i+1,:));
    elseif i==Size
        plocal(i,:)=chap8_8lbest(x(i-1,:),x(i,:),x(1,:));
        else 
        plocal(i,:)=chap8_8lbest(x(i-1,:),x(i,:),x(i+1,:));
    end
end

BestS=x(1,:);%初始化最优个体BestS
for i=2:Size
    if chap8_8func(x(i,:))>chap8_8func(BestS)
        BestS=x(i,:);
    end
end
...
%（4）判断和更新
       ...
       if chap8_8func(x(i,:))>p(i) %判断当此时的位置是否为最优的情况，当不满足时继续更新
          p(i)=chap8_8func(x(i,:));
          y(i,:)=x(i,:);
        end
        if p(i)>chap8_8func(BestS)
            BestS=y(i,:);
        end
    end
Best_value(kg)=chap8_8func(BestS);
end
figure(1);
kg=1:G;
