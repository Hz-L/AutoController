...
BestS=P(1,:); %全局最优个体
for i=2:Size
        if(chap8_16obj( P(i,1),P(i,2))>chap8_16obj( BestS(1),BestS(2)))
        BestS=P(i,:);
    end
end

fi=chap8_16obj( BestS(1),BestS(2));
...
%选择        
        if(chap8_16obj( v(i,1),v(i,2))>chap8_16obj( P(i,1),P(i,2)))
            P(i,:)=v(i,:);
        end
%判断和更新       
       if(chap8_16obj( P(i,1),P(i,2))>fi) %判断当此时的位置是否为最优的情况
          fi=chap8_16obj( P(i,1),P(i,2));
          BestS=P(i,:);
        end
    end
Best_f(kg)=chap8_16obj( BestS(1),BestS(2));
end
BestS     %最佳个体
Best_f(kg)%最大函数值
