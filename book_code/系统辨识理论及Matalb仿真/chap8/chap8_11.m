...
for i=1:Size
    Ji(i)=chap8_11obj(X(i,:),y,N);
    Xl(i,:)=X(i,:);     %Xl用于局部优化
end

BestS=X(1,:); %全局最优个体初始化
for i=2:Size
    if chap8_11obj(X(i,:),y,N)<chap8_11obj(BestS,y,N)
        BestS=X(i,:);
    end
end
%（3）进入主要循环，直到满足精度要求
...
%（4）判断和更新       
       if chap8_11obj(X(i,:),y,N)<Ji(i) %局部优化：判断当此时的位置是否为最优的情况
          Ji(i)=chap8_11obj(X(i,:),y,N);
          Xl(i,:)=X(i,:);
       end
        
        if Ji(i)<chap8_11obj(BestS,y,N)  %全局优化
          BestS=Xl(i,:);
        end
    end
Best_J(kg)=chap8_11obj(BestS,y,N);
end
